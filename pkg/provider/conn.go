...
func (g *GCPProvider) waitForDial(targetId string, dialTimeout time.Duration) error {
...
func (g *GCPProvider) getDockerClient(targetId string) (docker.IDockerClient, error) {
	tsnetConn, err := g.getTsnetConn()
	if err != nil {
		return nil, err
	}
